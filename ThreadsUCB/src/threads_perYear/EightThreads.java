...
public class EightThreads {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ExecutorService cityExecutor = Executors.newFixedThreadPool(8);
            ...
            int citiesPerThread = 40;  // 40 cidades por thread

            for (int i = 0; i < 8; i++) {
                int start = i * citiesPerThread;
                int end = Math.min(start + citiesPerThread, totalCities);

                if (start >= totalCities) break; // Para evitar exceção de índice
